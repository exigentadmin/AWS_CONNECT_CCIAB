...
resource "aws_connect_user" "ygrizzly" {
  instance_id        = aws_connect_instance.AWS-CONNECT-CCIAB-DEMO.id
  name               = "ygrizzly"
  password           = "Password123"
  routing_profile_id = data.aws_connect_routing_profile.Basic-Routing-Profile.routing_profile_id
...
resource "aws_connect_user" "fjournal" {
  instance_id        = aws_connect_instance.AWS-CONNECT-CCIAB-DEMO.id
  name               = "fjournal"
  password           = "Password123"
  routing_profile_id = data.aws_connect_routing_profile.Basic-Routing-Profile.routing_profile_id
...
resource "aws_connect_phone_number" "inbound-did" {
  target_arn   = aws_connect_instance.AWS-CONNECT-CCIAB-DEMO.arn
  country_code = "US"
  type         = "DID"
...
import {
  ...
  id = "5c70f824-4025-4b17-96e6-096dfd944787:c002e6c1-3ec2-47a6-909b-6e327a4adc0b"
}
output "agent-login" {
